  ...
  static getInstance(config?: MailServiceConfig): MailService {
    if (!MailService.instance && config) {
      ...
      MailService.instance = new MailService({
        ...
        fromEmail: import.meta.env.VITE_MAIL_FROM_EMAIL || 'noreply@civicconnect.local',
    ...
    try {
      ...
      const response = await fetch('https://api.sendgrid.com/v3/mail/send', {
        ...
        body: JSON.stringify({
          ...
          from: {
            email: this.config.fromEmail || 'noreply@civicconnect.local',
            name: this.config.fromName || 'Civic Connect',
          },
  ...
  async testConnection(testEmail: string): Promise<boolean> {
    try {
      const testPayload: EmailPayload = {
        to: testEmail,
        subject: 'Civic Connect - Mail Service Test',
        html: '<h1>Test Email</h1><p>This is a test email from Civic Connect mail service.</p>',
        text: 'Test Email\n\nThis is a test email from Civic Connect mail service.',
        fromName: this.config.fromName,
      };
